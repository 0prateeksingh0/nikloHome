...
interface CSPFlipbookViewerProps {
  pdfUrl: string;
  lightBox?: boolean;
...
const CSPFlipbookViewer: React.FC<CSPFlipbookViewerProps> = ({
  pdfUrl,
  lightBox = false,
  viewMode = '3d',
  height = '600px',
  ...
  useEffect(() => {
    ...
    const initializeFlipbook = () => {
      ...
      const options = {
        ...
        thumbsStyle: 'overlay',
        // Use CDN sources to avoid CSP issues
      ...
      if (window.jQuery?.fn?.flipBook) {
        window.jQuery(containerRef.current).flipBook(options);
      } else if (window.FlipBook) {
        new window.FlipBook(containerRef.current, options);
      } else {
...
export default CSPFlipbookViewer;
